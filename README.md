...
**SQL Method**
- **Filter subscription_events to the analytical window:** Keep only rows where **event_date** falls within 2025. Retain **subscription_id**, event_date, event_type, and mrr_change.
  - Truncate **event_date** to the first day of the month and name it mrr_month.
- **Aggregate base MRR by month:** From the filtered table, keep only rows where event_type is new. Group by **mrr_month** and sum **mrr_change** to produce **base_mrr** — the MRR generated by new subscriptions in that month, which becomes the starting base for measuring retention in subsequent months.
- **Aggregate expansion, contraction, and churn by month:** From the filtered table, keep only rows where event_type is expansion, contraction, or churn. Group by **mrr_month** and sum **mrr_change** to produce **net_existing_mrr** — the net revenue movement from existing customers in that month.
- **Join base MRR and existing MRR:** Join the base MRR and existing MRR tables on **mrr_month**.
  - Divide **net_existing_mrr** plus **base_mrr** by **base_mrr** and multiply by 100 to produce **nrr_pct** — the percentage of base MRR retained and expanded from existing customers in that month.
